...
ftrace_call_replace(unsigned long ip, unsigned long addr, int link)
{
	...
	create_branch(&op, (unsigned long *)ip, addr, link ? 1 : 0);

	return op;
}

static int
ftrace_modify_code(unsigned long ip, unsigned long old, unsigned long new)
{
	...
	if (patch_instruction((unsigned long *)ip, new))
		return -EPERM;
...
static int test_24bit_addr(unsigned long ip, unsigned long addr)
{
	...
	return create_branch(&op, (unsigned long *)ip, addr, 0) == 0;
}
		  ...
		  struct dyn_ftrace *rec, unsigned long addr)
{
	...
	if (patch_instruction((unsigned long *)ip, pop)) {
		  ...
		  struct dyn_ftrace *rec, unsigned long addr)
{
	...
	if (patch_instruction((unsigned long *)ip, op))
		return -EPERM;
...
static int setup_mcount_compiler_tramp(unsigned long tramp)
{
	...
	if (patch_branch((unsigned long *)tramp, ptr, 0)) {
...
static int __ftrace_make_nop_kernel(struct dyn_ftrace *rec, unsigned long addr)
{
	...
	if (patch_instruction((unsigned long *)ip, ppc_inst(PPC_INST_NOP))) {
		pr_err("Patching NOP failed.\n");
		return -EPERM;
	}
...
__ftrace_make_call(struct dyn_ftrace *rec, unsigned long addr)
{
	...
	err = create_branch(&op, (unsigned long *)ip,
					...
					unsigned long addr)
{
	...
	if (test_24bit_addr(ip, addr)) {
		/* within range */
		if (patch_branch((unsigned long *)ip, addr, BRANCH_SET_LINK)) {
			pr_err("REL24 out of range!\n");
			return -EINVAL;
		}
	...
	/* Ensure branch is within 24 bits */
	if (create_branch(&op, (unsigned long *)ip, tramp, BRANCH_SET_LINK)) {
		pr_err("Branch out of range\n");
		return -EINVAL;
	}

	if (patch_branch((unsigned long *)ip, tramp, BRANCH_SET_LINK)) {
		pr_err("REL24 out of range!\n");
		return -EINVAL;
	}
...
int ftrace_update_ftrace_func(ftrace_func_t func)
{
	...
	old = ppc_inst_read((unsigned long *)&ftrace_call);
	...
	if (!ret) {
		...
		old = ppc_inst_read((unsigned long *)&ftrace_regs_call);
		new = ftrace_call_replace(ip, (unsigned long)func, 1);
		ret = ftrace_modify_code(ip, old, new);
	}
#endif
